...
# -*- coding: utf-8 -*-
"""
GENERADOR DE MATRICES DE CONFUSIÓN - ÓRGANOS (TEJIDOS)
...
NOMBRES_TARG = ['Gallbladder', 'Cystic Plate', 'Cystic Duct', 'Cystic Artery', 
                'Cystic Pedicle', 'Blood Vessel', 'Fluid', 'Adhesion', 
                'Omentum', 'Liver', 'Gut', 'Specimen Bag', 
                'Abdominal Wall', 'Gauze', 'Null']
...
# 15 clases para los órganos
metricas = {i: {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0} for i in range(15)}

print("\n-> Procesando videos para Matrices de Confusión (Órganos)...")
...
with torch.no_grad():
    for nombre in TEST_VIDEOS:
        ...
        # Etiquetas Reales de ÓRGANOS
        target_labels = np.loadtxt(os.path.join(DATA_DIR, "target", f"{nombre}.txt"), dtype=int, delimiter=",")[:, 1:]
        
        T = min(cams_i.shape[0], pesos_t.shape[0], len(target_labels))
        
        # Inferencia
        ci, cv, ct = torch.from_numpy(cams_i[:T]).to(device), torch.from_numpy(cams_v[:T]).to(device), torch.from_numpy(cams_t[:T]).to(device)
        _, _, r_t  = modelo(ci, cv, ct, pesos_i[:T], pesos_v[:T], pesos_t[:T])
        
        # Binarizar predicciones (Umbral 0.5)
        probs_t = cam_to_prob(r_t).cpu().numpy()
        preds_t = (probs_t >= 0.5).astype(int)
        y_true  = target_labels[:T]
        
        # Acumular resultados por clase (15 clases)
        for c in range(15):
            metricas[c]['TP'] += np.sum((preds_t[:, c] == 1) & (y_true[:, c] == 1))
            metricas[c]['TN'] += np.sum((preds_t[:, c] == 0) & (y_true[:, c] == 0))
            metricas[c]['FP'] += np.sum((preds_t[:, c] == 1) & (y_true[:, c] == 0))
            metricas[c]['FN'] += np.sum((preds_t[:, c] == 0) & (y_true[:, c] == 1))
...
for idx, ax in enumerate(axes.flatten()):
    ...
    # Paleta de tonos cálidos para los tejidos
    sns.heatmap(cm, annot=True, fmt='d', cmap='Reds', cbar=False, ax=ax, annot_kws={"size": 11})
    
    ax.set_title(NOMBRES_TARG[idx], fontsize=12, fontweight='bold')
    ax.set_xticklabels(['Ausente', 'Presente'], fontsize=9)
    ax.set_yticklabels(['Ausente', 'Presente'], fontsize=9)
    
    if idx >= 10: # Solo poner label X en la última fila
...
plt.suptitle('Matrices de Confusión de Órganos y Tejidos - SpatioTemporalRDV', fontsize=18, fontweight='bold', y=0.98)
plt.show()
